...
void updateSystem() {
    ...
    for( unsigned int v = 0 ; v < mesh.V.size() ; ++v ) {
        ...
        it != edgeAndVertexWeights.get_weight_of_adjacent_edges_it_end(v) ; ++it) {
            ...
            // WHAT TO PUT HERE ??????? How to update the entries of A ?a
        }
    }
    std::cout<<"i"<<std::endl;
    for( unsigned int v = 0 ; v < mesh.V.size() ; ++v ) {
...
void updateMeshVertexPositionsFromARAPSolver() {
    ...
    for( unsigned int arapIteration = 0 ; arapIteration < maxIterationsForArap ; ++arapIteration ) {
        ...
        for( unsigned int v = 0 ; v < mesh.V.size() ; ++v ) {
            ...
            for( std::map< unsigned int , double >::const_iterator it = edgeAndVertexWeights.get_weight_of_adjacent_edges_it_begin(v) ;
                 it != edgeAndVertexWeights.get_weight_of_adjacent_edges_it_end(v) ; ++it) {
            // std::cout<<"aaaaa "<<edgeAndVertexWeights.sumVertexWeights()<<std::endl;
            // for(int i=0;i<edgeAndVertexWeights.get_n_adjacent_edges(v);++i) {
            //     std::map< unsigned int , double >::const_iterator it=edgeAndVertexWeights.get_weight_of_adjacent_edges_it_begin(v);
        ...
        // std::cout<<equationIndex<<std::endl;
        for( unsigned int v = 0 ; v < mesh.V.size() ; ++v ) {
            if(verticesHandles[v] != -1) {
        ...
        // Once the matrix A and the vector B are correctly set, we obtain the position of the vertices by solving for A.X = Ba
        ...
        for( unsigned int v = 0 ; v < mesh.V.size() ; ++v ) {
            ...
            // std::cout<<tensorMatrix<<std::endl;
...
int main (int argc, char ** argv) {
    ...
    std::cout<<edgeAndVertexWeights.get_n_adjacent_edges(00)<<std::endl;
...
//Exercice 1: x0=0.5 x1=0.5 x2=-0.5
